...
// Hash slug to consistently pick a unique photo from the pool
function getCoverImageUrl(slugOrTopic = '') {
  let hash = 0;
  for (let i = 0; i < slugOrTopic.length; i++) {
    hash = (hash * 31 + slugOrTopic.charCodeAt(i)) & 0xffffffff;
  }
  const idx = Math.abs(hash) % PHOTO_POOL.length;
  return `https://images.unsplash.com/photo-${PHOTO_POOL[idx]}?w=1200&h=630&fit=crop&auto=format&q=80`;
}
...
function parseArticle(content) {
  ...
  for (const line of lines) {
    if (!title && line.startsWith('# ')) {
      ...
      metaDescription = line.replace(/^META:\s*/, '').trim();
    } else {
      bodyLines.push(line);
    }
  }
  ...
  // Excerpt: first non-empty, non-heading paragraph — max 200 chars
  const excerpt = bodyLines
    .map(l => l.trim())
    .find(l => l && !l.startsWith('#') && !l.startsWith('META:') && !l.startsWith('INTERNAL LINK:') && l.length > 40)
